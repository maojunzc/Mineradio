...
- 🎵 新增酷我音乐搜索与播放（含 Cookie 持久化登录）
- 🎵 新增酷狗音乐搜索与播放（含 Cookie 持久化登录）
- 📝 新增本地 LRC 歌词导入功能（.lrc 文件解析 + localStorage 存储）
- 🔄 自定义歌词自动覆盖：播放时优先显示用户导入的歌词
- 🐛 修复网易云网盘 M4A 歌词无法加载的问题
- 🐛 修复酷我/酷狗搜索结果在 All 模式下不显示的问题
- 🐛 修复网易云普通 VIP 会员被错误标记为 SVIP 的问题
- 🐛 修复手动播放时 AudioContext 恢复时序问题
- 🔤 新增拼音首字母搜索（输入 smfx 可搜索"失眠飞行"）
